...
/**
 * Formaterar kommentarer snyggt.
 * Gör om @param och @returns till punktlistor.
 */
...
function extractDocs(content) {
  ...
  // 2. Hitta ALLA exporter (även de utan kommentarer)
  const exportRegex =
  ...
  while ((match = exportRegex.exec(remainingContent)) !== null) {
    ...
    if (commentBlock && commentBlock.trim().length > 0) {
      ...
      // Här fångar vi de o-dokumenterade!
      items.push({
        name,
        signature: fullSignature,
